...
class Scanner:
    ...
    def __init__(self, symbol_table, file, identifier_file, string_file, int_file):
        self.symbol_table = symbol_table
        self.file = file
        self.token_list = []
        self.separator_list = []
        self.pif_table = []
        self.identifier_fa = FiniteAutomata(identifier_file)
        self.string_fa = FiniteAutomata(string_file)
        self.int_fa = FiniteAutomata(int_file)
    ...
    def scan(self):
        self.identifier_fa.read_fa()
        self.int_fa.read_fa()
        self.string_fa.read_fa()
    ...
    def is_int_constant_fa(self, word):
        if self.int_fa.check_sequence(word):
            print("Int Constant: " + word + "\n")
            return True

        return False

    def is_identifier_fa(self, word):
        if self.identifier_fa.check_sequence(word):
            print("Identifier: " + word + "\n")
            return True

        return False

    def is_string_constant_fa(self, word):
        if self.string_fa.check_sequence(word):
            print("String Constant: " + word + "\n")
            return True
